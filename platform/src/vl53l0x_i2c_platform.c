...
char  debug_string[VL53L0X_MAX_STRING_LENGTH_PLT]; // @suppress("Symbol is not resolved")
...
bool_t _check_min_version(void)
{
    ...
    COMMS_VERSION_INFO comms_version_info; // @suppress("Type cannot be resolved")
    ...
    else
    {
        ...
        if (
            (comms_version_info.major > MIN_COMMS_VERSION_MAJOR) || // @suppress("Field cannot be resolved")
            ((comms_version_info.major == MIN_COMMS_VERSION_MAJOR) && (comms_version_info.minor > MIN_COMMS_VERSION_MINOR)) || // @suppress("Field cannot be resolved")
            ((comms_version_info.major == MIN_COMMS_VERSION_MAJOR) && (comms_version_info.minor == MIN_COMMS_VERSION_MINOR) && (comms_version_info.build >= MIN_COMMS_VERSION_BUILD)) // @suppress("Field cannot be resolved")
            )
        {
        ...
        if ( comms_version_info.revision < MIN_COMMS_VERSION_REVISION ) // @suppress("Field cannot be resolved")
...
int32_t VL53L0X_comms_initialise(uint8_t comms_type, uint16_t comms_speed_khz)
{
    ...
    {
		int32_t STATUS_DLL_INIT_FAILED;
...
int32_t VL53L0X_platform_wait_us(int32_t wait_us)
{
     ...
     */
    HANDLE hEvent = CreateEvent(0, TRUE, FALSE, 0); // @suppress("Type cannot be resolved")
