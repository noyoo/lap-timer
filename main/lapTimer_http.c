...
static esp_err_t update_uri_handler(httpd_req_t* request) {
    ...
    for (int i = 0; i <= slaves.pointer; i++) {
        char* buffer = NULL;
        uint8_t ret = asprintf(&buffer, ",%d", (uint8_t)slaves.slave[i]._IRstate);
        if (ret > 0) {
            httpd_resp_sendstr_chunk(request, buffer);
        }
        free(buffer);
