...
static esp_err_t datastream_uri_handler(httpd_req_t* request) {
    size_t len_IP = httpd_req_get_hdr_value_len(request, "SlaveIP");
    if (len_IP > 0) {
        ...
        size_t len_status = httpd_req_get_hdr_value_len(request, "Status");
        char* SlaveStatus_string[len_status];
        uint8_t SlaveStatus = atoi(SlaveStatus_string);
        httpd_req_get_hdr_value_str(request, "Status", SlaveStatus_string, len_status);
        if (checkIfSlaveRegistered(SlaveIP)) {
            updateSlaveStatus(SlaveIP, SlaveStatus);
        } else {
            registerSlave(SlaveIP, SlaveStatus);
        }
...
void http_client_poll(void) {
    ...
    int64_t contentLength = esp_http_client_fetch_headers(http_client);
